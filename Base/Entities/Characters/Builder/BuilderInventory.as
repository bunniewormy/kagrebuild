...
void MakeBlocksMenu(CInventory@ this, const Vec2f &in INVENTORY_CE)
{
	...
	if (menu !is null)
	{
		...
		const Vec2f MISC_POS = menu.getLowerRightPosition() + Vec2f(30, 110);

		CGridMenu@ misc = CreateGridMenu(MISC_POS, blob, Vec2f(1, 1), "Misc Slot");
		if (misc !is null)
		{
			misc.SetCaptionEnabled(true);

			bool misc_in_hand = false;
			bool misc_in_inv = false;

			CBlob@ held = this.getBlob().getCarriedBlob();
			...
			AttachmentPoint@ ap = this.getBlob().getAttachments().getAttachmentPointByName("MISC");

			CBlob@ current_misc_item = ap.getOccupied();

			if (ap !is null)
			{
				if (current_misc_item !is null)
				{
					misc_in_inv = true;
				}
			}
			...
			if (misc_in_hand)
			{
				...
				string misc_icon_name;
				u8 icon_frame;

				if (misc_in_inv) 
				{
					misc_icon_name = current_misc_item.inventoryIconName + ".png";
					misc_item_name = current_misc_item.getInventoryName();
					icon_frame = current_misc_item.inventoryIconFrame;
				}
				else 
				{
					misc_icon_name = "ossas";
					misc_item_name = getTranslatedString("Store misc item");
					icon_frame = 0;
				}

				CGridButton@ clear = misc.AddButton(misc_icon_name, icon_frame, Vec2f(16, 16), misc_item_name, Builder::STORE_MISC_ITEM, Vec2f(1, 1), params);
				if (clear !is null)
				{
					clear.deleteAfterClick = false;
					clear.selectOnClick = false;
					clear.SetHoverText(getTranslatedString("Misc item slot.\nCan hold a:\nsponge\nlantern\nbucket\nchicken"));
				}
			}
			...
			else if (misc_in_inv)
			{
				...
				CGridButton@ clear = misc.AddButton(current_misc_item.inventoryIconName + ".png", current_misc_item.inventoryIconFrame, Vec2f(16, 16), current_misc_item.getInventoryName(), Builder::TAKE_MISC_ITEM, Vec2f(1, 1), params);
				if (clear !is null)
				{
					clear.deleteAfterClick = false;
					clear.selectOnClick = false;
...
void onCommand(CInventory@ this, u8 cmd, CBitStream@ params)
{
	...
	else if (cmd == Builder::STORE_MISC_ITEM)
	{
		u16 id;
		if (!params.saferead_u16(id)) return;

		u16 held_id;
		if (!params.saferead_u16(held_id)) return;

		CBlob@ target = getBlobByNetworkID(id);
		if (target is null) return;

		CBlob@ held = getBlobByNetworkID(held_id);
		if (held is null) return;

		AttachmentPoint@ ap = target.getAttachments().getAttachmentPointByName("MISC");

		if(ap !is null)
		{
			CBlob@ misc = target.getAttachments().getAttachmentPointByName("MISC").getOccupied();
			if(ap !is null && misc !is null)
			{
				misc.server_DetachFrom(target);
				target.server_AttachTo(misc, "PICKUP");

				misc.SetVisible(true);
			}

			held.server_DetachFrom(target);
			target.server_AttachTo(held, ap);

			held.SetVisible(false);

			printf("name " + ap.getOccupied().getName());
		}
	}
	else if (cmd == Builder::TAKE_MISC_ITEM)
	{
		u16 id;
		if (!params.saferead_u16(id)) return;
		...
		CBlob@ misc = target.getAttachments().getAttachmentPointByName("MISC").getOccupied();
		AttachmentPoint@ ap = target.getAttachments().getAttachmentPointByName("PICKUP");

		if(ap !is null && misc !is null)
		{
			misc.server_DetachFrom(target);
			target.server_AttachTo(misc, ap);
