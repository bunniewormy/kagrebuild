...
{
	int x = Maths::Floor(size.x) - 1;
	int y = Maths::Floor(size.y) - 1;
	...
	for (int j = 0; j < y; j++)
	{
		for (int i = 0; i < x; i++)
		{
			...
			if (b_top && b_left) frame = topleft;
			else if (b_top && b_right) frame = topright;
			else if (b_bottom && b_left) frame = bottomleft;
			else if (b_bottom && b_right) frame = bottomright;
			else if (b_top) frame = top;
			else if (b_bottom) frame = bottom;
			else if (b_right) frame = right;
			else if (b_left) frame = left;
			else frame = mid;

			GUI::DrawIcon("CrateSlots.png", frame, Vec2f(8, 8), pos + Vec2f(i * getMap().tilesize, j * getMap().tilesize) * zoom * 2, zoom);
		}
	}
}
