...
shared class CTFSpawns : RespawnSystem
{
	...
	CBlob@ getSpawnBlob(PlayerInfo@ p_info)
	{
		...
		return server_CreateBlob("tent", 255, Vec2f(1, 1));
	}
	...
	void AddPlayerToSpawn(CPlayer@ player)
	{
		...
		if (getSpawnBlob(info).exists("additional respawn time"))
		{
			tickspawndelay += getSpawnBlob(info).get_u8("additional respawn time");
		}

		CBlob@ oldSpawn = getSpawnBlob(info);

		//clamp it so old bad values don't get propagated
		s32 old_spawn_time = Maths::Max(0, info.can_spawn_time);
